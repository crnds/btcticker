...
| Fees | mempool.space `GET /api/v1/fees/mempool-blocks` | 60 s | Same tier derivation as `shared.js`'s `BTC.fees.deriveTiers()` (moved there from `app.js` — both the ticker and dashboard call the shared copy now): high = block[0].medianFee, med = block[2], low = block[5], no-priority = last block (indices clamped to array length). Parse only `medianFee` per block — stream-parse or use a JSON filter, the full response is large. |
...
| CDC strip | Kraken `GET /0/public/OHLC?pair=XBTUSD&interval=1440` | 1 h | Compute on-device, porting `calc_ema()` from `fetch_cdc.py` (the browser's own copy in `app.js` was removed — it was dead code, since `app.js` only ever reads the pre-computed `data/cdc.js` file and never fetches Kraken itself) + the block-building loop in `fetch_cdc.py:main()`: last 100 daily closes → EMA12/EMA26 → last 30 days, `bull = ema12 > ema26`, bar height ∝ `|ema12 − ema26|` normalized over the 30-day window. Needs ≥56 closes (30 blocks + EMA26 warmup). A frozen golden-vector fixture for this recurrence already exists at `tests/fixtures/ema_golden.json` — reuse it as the CYD port's acceptance test rather than hand-deriving new expected values. |
