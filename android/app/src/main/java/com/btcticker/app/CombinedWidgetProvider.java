...
public class CombinedWidgetProvider extends AppWidgetProvider {
    ...
    // Share caches with the individual widgets
    ...
    public void onReceive(Context ctx, Intent intent) {
        ...
        int[] ids = mgr.getAppWidgetIds(new ComponentName(ctx, CombinedWidgetProvider.class));
    ...
    private void fetchAndUpdate(Context ctx, AppWidgetManager mgr, int[] ids) {
        ...
        executor.execute(() -> {
            ...
            long    cdcUpdated = cPrefs.getLong("last_update", 0);
            Bitmap  strip      = blocks != null ? renderStrip(blocks, ctx) : null;

            final String fPrice = dp; final String fChange = dc; final float fChgF = df;
            final Bitmap fStrip = strip; final long fCdcAt = cdcUpdated;

            handler.post(() -> {
                RemoteViews v = new RemoteViews(ctx.getPackageName(), R.layout.widget_combined);
                v.setTextViewText(R.id.widget_combined_price,  fPrice);
                v.setTextViewText(R.id.widget_combined_change, fChange);
                v.setTextColor(R.id.widget_combined_change,
                    fChgF >= 0 ? Color.parseColor("#00e676") : Color.parseColor("#ff1744"));
                if (fStrip != null) v.setImageViewBitmap(R.id.widget_combined_strip, fStrip);
                v.setTextViewText(R.id.widget_combined_cdc_label,
                    "CDC · " + (fCdcAt > 0 ? PriceWidgetProvider.timeAgo(fCdcAt) : "–"));
                v.setOnClickPendingIntent(R.id.widget_combined_refresh_btn, manualRefreshIntent(ctx));
    ...
    // ── Strip bitmap renderer for the right pane ─────────────

    private Bitmap renderStrip(List<CdcWidgetProvider.Block> blocks, Context ctx) {
        float density = ctx.getResources().getDisplayMetrics().density;

        // Right pane is ~90dp wide, strip area is ~40dp tall after label
        int W = Math.round(70 * density);
        int H = Math.round(40 * density);
        ...
        int   n       = blocks.size();
        float gap     = 0f;              // no gaps — squeeze all bars into one row
    ...
    private PendingIntent alarmIntent(Context ctx) {
        Intent i = new Intent(ctx, CombinedWidgetProvider.class);
        i.setAction(ACTION_REFRESH);
        return PendingIntent.getBroadcast(ctx, 50, i,
    ...
    private PendingIntent manualRefreshIntent(Context ctx) {
        ...
        return PendingIntent.getBroadcast(ctx, 51, i,
            PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
    }
}
